...
public class Attack extends GameController {
    ...
    private boolean isCaseValide(Case target) {
        if(target.getTouched()) {
            resultShoot("Cette case a déjà été touchée !!");
            return false;
        }
        isOccupied(target);
        return true;
    ...
    private void isOccupied(Case target) {
        if(target.getOccupiedBy() != null) {
            resultShoot(target.getOccupiedBy().getType() + " touché !");
            updateColorCase(true, target);
            return;
        }
        resultShoot("Aucune cible touchée . . .");
        updateColorCase(false, target);
    }
}
